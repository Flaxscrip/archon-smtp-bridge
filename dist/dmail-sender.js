...
export class DMailSender {
     ...
     */
    emailToDMail(email, recipientDids, replyToken, useProperties = false) {
        const bodyParts = [];
     ...
     */
    async setBridgeProperties(dmailDid, metadata) {
        if (!this.keymaster)
            return false;
        try {
            const data = {
            ...
            console.log(`[DMail] Could not set properties (asset properties may not be enabled):`, error instanceof Error ? error.message : String(error));
            return false;
        }
    }
    /**
     ...
     */
    async processEmail(email, recipientDids, record, replyToken, threadId) {
...
//# sourceMappingURL=dmail-sender.js.map